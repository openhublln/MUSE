...
class CA_CFAR():
    ...
    def __init__(self, win_param, rd_size, threshold=None, threshold_near=None, threshold_far=None):
        ...
        if self.threshold_near is not None and self.threshold_far is not None:
            thresholds = np.linspace(self.threshold_near, self.threshold_far, rd_size[1])
            self.threshold = 10 ** (thresholds / 10)
        else:
    ...
    def __call__(self, rd_matrix):
        ...
        if self.threshold_near is not None and self.threshold_far is not None:
            hit_matrix = rd_snr > self.threshold[np.newaxis, :]
        else:
